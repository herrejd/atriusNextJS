...
**File: `app/api/directions/route.js`** (Get directions between two locations)
...
Get directions between two POIs:
```bash
curl "http://localhost:3000/api/directions?from=1025&to=551"
```

Get accessible directions using coordinates:
```bash
curl "http://localhost:3000/api/directions?from=36.085,-115.150,1&to=551&accessible=true"
```
